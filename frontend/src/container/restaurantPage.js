...
const RestaurantPage = () => {
    ...
    const getInfo = async () => {
        ...
        const {data: {message, contents}} = await instance.get('/getInfo', { params:{  
            id: id }   
        } )
        setInfo(contents)
        console.log(message, contents.name)
        getComments()
    }
    const getComments = async () => {
        // TODO Part III-3: get a restaurant's comments 
        const {data: {message, contents}} = await instance.get('/getCommentsByRestaurantId', { params:{  
            restaurantId: id }   
        } )
